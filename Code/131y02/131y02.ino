  ...
  if (newPosition != oldPosition) {
    switch (menu) {
      ...
      case 2: //Настройка IF-SHIFT
        if (newPosition > oldPosition && varinfo.ifshift <= 3000) {
          if (varinfo.ifshift % ifstep) {
            varinfo.ifshift = varinfo.ifshift + ifstep - (varinfo.ifshift % ifstep);
          }
          else {
            varinfo.ifshift = varinfo.ifshift + ifstep;
          }
        }
        if (newPosition < oldPosition && varinfo.ifshift >= -3000) {
          if (varinfo.ifshift % ifstep) {
            varinfo.ifshift = varinfo.ifshift - (varinfo.ifshift % ifstep);
          }
          else {
            varinfo.ifshift = varinfo.ifshift - ifstep;
          }
...
void vfosetup() {
  ...
  si.set_freq((varinfo.lofreq + RXifshift), 0, (varinfo.freq + varinfo.lofreq + RXifshift));
}
