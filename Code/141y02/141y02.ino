...
#define bfo_freq 10000000UL // Частота ГОЧ.
#define min_hardware_freq 10 // *100KHz Минимальный железный предел частоты диапазона VFO
#define max_hardware_freq 105 // *100KHz Максимальный железный предел частоты диапазона VFO
...
struct general_set {
  ...
  int bfo_cal_freq_set  = 0; // калибровка опоры 10МГц.
...
#define bfo_cal_freq general_setting.bfo_cal_freq_set
...
void readencoder() { // работа с енкодером
  ...
  if (newPosition != oldPosition) { // ЕСЛИ КРУТИЛИ энкодер
    ...
    switch (menu) {
      ...
      case 9: //Настройка опорного гетеродина ГОЧ
        if (newPosition > oldPosition) bfo_cal_freq += arraystp[stp];
        if (newPosition < oldPosition) bfo_cal_freq -= arraystp[stp];
        vfosetup();
        break;
...
void mainscreen() { //Процедура рисования главного экрана
  ...
  switch (menu) {
    ...
    case 9: //Меню 8 - настройка ГОЧ
      display.println(bfo_cal_freq);
      display.setTextSize(1);
      display.print(menu);
      display.print("  BFO cal ");
      display.print((char)240);
      display.print("Hz");
      break;

    case 10: //Меню 9 - Настройка калибровки кварца
    ...
    case 11: //Меню 10 - Настройка калибровки по питанию
    ...
    case 12: //Меню 14 - Количество диапазонов
...
void vfosetup() {
  if (mode) {
    si.set_freq((lo_usb_freq + RXifshift), 0, (vfo_freq + lo_usb_freq + bfo_freq + bfo_cal_freq + RXifshift));
  }
  else {
    si.set_freq((lo_lsb_freq + RXifshift), 0, (vfo_freq + lo_lsb_freq + bfo_freq + bfo_cal_freq + RXifshift));
  }
}
